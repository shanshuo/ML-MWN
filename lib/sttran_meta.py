...
from lib.meta_layers import MetaModule, MetaSequential, MetaConv2d, MetaBatchNorm2d, MetaLinear, MetaEmbedding
from lib.word_vectors import obj_edge_vectors
from lib.transformer_meta import transformer
...
class ObjectClassifier(nn.Module):
    ...
    def forward(self, entry):
        ...
        if self.mode  == 'predcls':
            ...
            obj_embed = entry['distribution'] @ self.obj_embed.weight  # @: torch.matmul()
            ...
            if self.training:
                ...
                box_idx = entry['boxes'][:, 0].long()
                b = int(box_idx[-1] + 1)
                # https://github.com/yrcong/STTran/issues/18#issuecomment-952022825
...
class STTran(MetaModule):
    ...
    def __init__(self, mode='sgdet',
        ...
        self.conv = MetaSequential(
            MetaConv2d(2, 256 //2, kernel_size=7, stride=2, padding=3, bias=True),
            nn.ReLU(inplace=True),
            MetaBatchNorm2d(256//2, momentum=0.01),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            MetaConv2d(256 // 2, 256, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(inplace=True),
            MetaBatchNorm2d(256, momentum=0.01),
        )
        self.subj_fc = MetaLinear(2048, 512)
        self.obj_fc = MetaLinear(2048, 512)
        self.vr_fc = MetaLinear(256*7*7, 512)

        embed_vecs = obj_edge_vectors(obj_classes, wv_type='glove.6B', wv_dir='data', wv_dim=200)
        self.obj_embed = MetaEmbedding(len(obj_classes), 200)
        self.obj_embed.weight.data = embed_vecs.clone()

        self.obj_embed2 = MetaEmbedding(len(obj_classes), 200)
        ...
        self.a_rel_compress = MetaLinear(1936, self.attention_class_num)  # 3: looking at, not looking at, unsure
        self.s_rel_compress = MetaLinear(1936, self.spatial_class_num)  # 6: in front of, behind, on the side of, above, beneath, in
        self.c_rel_compress = MetaLinear(1936, self.contact_class_num)  # 17: carrying, drinking from, have it on the back, ...
        # print('remove semantic feature')
        # self.a_rel_compress = nn.Linear(1536, self.attention_class_num)  # 3: looking at, not looking at, unsure
        # self.s_rel_compress = nn.Linear(1536, self.spatial_class_num)  # 6: in front of, behind, on the side of, above, beneath, in
        # self.c_rel_compress = nn.Linear(1536, self.contact_class_num)  # 17: carrying, drinking from, have it on the back, ...

    def forward(self, entry):

        entry = self.object_classifier(entry)
        # entry['features']: [bbox_num, 2048]
        # visual part
        subj_rep = entry['features'][entry['pair_idx'][:, 0]]  # [pair_num, 2048], [:, 0] for person
        subj_rep = self.subj_fc(subj_rep)  # [pair_num, 512], the number of person-object pairs in this video
        assert not torch.any(torch.isnan(subj_rep))
        obj_rep = entry['features'][entry['pair_idx'][:, 1]]  # [:, 1] for object
        obj_rep = self.obj_fc(obj_rep)  # [pair_num, 512]
        assert not torch.any(torch.isnan(obj_rep))
        vr = self.union_func1(entry['union_feat'])+self.conv(entry['spatial_masks'])  # [pair_num, 256, 7, 7]
        vr = self.vr_fc(vr.view(-1,256*7*7))  # [pair_num, 512]
        assert not torch.any(torch.isnan(vr))
        x_visual = torch.cat((subj_rep, obj_rep, vr), 1)  # [pair_num, 512+512+512=1536]

        # semantic part
        subj_class = entry['pred_labels'][entry['pair_idx'][:, 0]]  # [pair_num]
        obj_class = entry['pred_labels'][entry['pair_idx'][:, 1]]  # [pair_num]
        subj_emb = self.obj_embed(subj_class)  # [pair_num, 200]
        assert not torch.any(torch.isnan(subj_emb))
        obj_emb = self.obj_embed2(obj_class)  # [pair_num, 200]
        assert not torch.any(torch.isnan(obj_emb))
        x_semantic = torch.cat((subj_emb, obj_emb), 1)  # [pair_num, 200+200=400]

        rel_features = torch.cat((x_visual, x_semantic), dim=1)  # [pair_num, 1536+400=1936]
        # rel_features = x_visual
