...
import copy
from lib.meta_layers import MetaModule, MetaMultiheadAttention, MetaLinear, MetaLayerNorm

class TransformerEncoderLayer(MetaModule):

    def __init__(self, embed_dim=1936, nhead=4, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        self.self_attn = MetaMultiheadAttention(embed_dim, nhead, dropout=dropout)

        self.linear1 = MetaLinear(embed_dim, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = MetaLinear(dim_feedforward, embed_dim)

        self.norm1 = MetaLayerNorm(embed_dim)
        self.norm2 = MetaLayerNorm(embed_dim)
    ...
    def forward(self, src, input_key_padding_mask):
        ...
        assert not torch.any(torch.isnan(src2))
        assert not torch.any(torch.isnan(local_attention_weights))
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        assert not torch.any(torch.isnan(src))
...
class TransformerDecoderLayer(MetaModule):

    def __init__(self, embed_dim=1936, nhead=4, dim_feedforward=2048, dropout=0.1):
        super().__init__()

        self.multihead2 = MetaMultiheadAttention(embed_dim, nhead, dropout=dropout)
        ...
        self.norm3 = MetaLayerNorm(embed_dim)
        self.dropout2 = nn.Dropout(dropout)
    ...
    def forward(self, global_input, input_key_padding_mask, position_embed):
        ...
        assert not torch.any(torch.isnan(tgt2))
        assert not torch.any(torch.isnan(global_attention_weights))
...
class TransformerEncoder(MetaModule):
...
class TransformerDecoder(MetaModule):
...
class transformer(nn.Module):
    ...
    def __init__(self, enc_layer_num=1, dec_layer_num=3, embed_dim=1936, nhead=8, dim_feedforward=2048,
        ...
        self.position_embedding = MetaEmbedding(2, embed_dim) #present and next frame
        nn.init.uniform_(self.position_embedding.weight)


    def forward(self, features, im_idx):  # features: [num_pair, 1936], im_idx: [num_pair]
        rel_idx = torch.arange(im_idx.shape[0])  # [num_pair]
        # im_idx: [num_pair], save this video's person-object pairs' image id
        l = torch.sum(im_idx == torch.mode(im_idx)[0])  # the highest box number in the single frame, e.g.3
        b = int(im_idx[-1] + 1)  # num_frame
        rel_input = torch.zeros([l, b, features.shape[1]]).to(features.device)  # l*b >= num_pair
        ...
        for i in range(b):
            ...
            masks[i, torch.sum(im_idx == i):] = 1  # if this frame's pairs < l, then mask the remaining as 1 since features are all zeros
        
        # rel_input: [l, b, 1936], masks: [b, l]
        # the input sequence of transformer: the person-object pairs in a frame
        # some frames' number of pairs may be less than l, so needs masks (=1 means ignore this pair)
        # spatial encoder 
        local_output, local_attention_weights = self.local_attention(rel_input, masks)  # [L, B, C]
        assert not torch.any(torch.isnan(local_output))
        assert not torch.any(torch.isnan(local_attention_weights))
        local_output = (local_output.permute(1, 0, 2)).contiguous().view(-1, features.shape[1])[masks.view(-1) == 0]  # [pair_num, 1936]

        global_input = torch.zeros([l * 2, b - 1, features.shape[1]]).to(features.device)  # l*2: this frame and next frame's boxes (need mask), b-1: second last frame and last frame
        ...
        for j in range(b - 1):  # current frame and next frame
            ...
            assert not torch.any(torch.isnan(position_embed))
            position_embed[torch.sum(im_idx == j):torch.sum(im_idx == j)+torch.sum(im_idx == j+1), j, :] = self.position_embedding.weight[1]
        # https://discuss.pytorch.org/t/check-if-at-least-one-element-in-tensor-is-nonzero/125426/2
        global_masks = (torch.sum(global_input.view(-1, features.shape[1]).abs(), dim=1) == 0).view(l * 2, b - 1).permute(1, 0)  # [frame_num-1, frame_pair_num*2], 2 is windows size
        # temporal decoder
        ...
        assert not torch.any(torch.isnan(global_attention_weights))
        output = torch.zeros_like(features)  # [num_pair, 1936]
