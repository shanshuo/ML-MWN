...
class BasicSceneGraphEvaluator:
    ...
    def print_stats(self):
        ...
        for k, v in self.result_dict[self.mode + '_recall'].items():
            print('R@%i: %f' % (k, np.mean(v)))  # mean recall over all annotated frames
    ...
    def evaluate_scene_graph(self, gt, pred):
        ...
        for idx, frame_gt in enumerate(gt):
            ...
            pred_scores_1 = np.concatenate((pred['attention_distribution'][pred['im_idx'] == idx].cpu().numpy(),  # only attention scores, spatial and contact set as 0
            ...
            if self.mode == 'predcls':
                ...
                pred_entry = {
                    ...
                    'rel_scores': np.concatenate((pred_scores_1, pred_scores_2, pred_scores_3), axis=0)  # (1, 26); 26: 25+otherrelationship
                }
            else:
                pred_entry = {
                    'pred_boxes': pred['boxes'][:, 1:].cpu().clone().numpy(),
