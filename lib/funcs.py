...
def assign_relations(prediction, gt_annotations, assign_IOU_threshold):
    ...
    for i, j in enumerate(gt_annotations):  # frame i

        gt_boxes = np.zeros([len(j), 4])
        gt_labels = np.zeros(len(j))
        gt_boxes[0] = j[0]['person_bbox']  # assume only one person in a frame
        gt_labels[0] = 1
        for m, n in enumerate(j[1:]):  # ignore person box
            gt_boxes[m+1,:] = n['bbox']
            gt_labels[m+1] = n['class']

        pred_boxes = FINAL_BBOXES[FINAL_BBOXES[:,0] == i, 1:].detach().cpu().numpy()  # [object_box_num,4], ignore person box
        #labels = FINAL_LABELS[FINAL_BBOXES[:,0] == i].detach().cpu().numpy()

        IOUs = bbox_overlaps(pred_boxes, gt_boxes)  # [pred_box, gt_box]
